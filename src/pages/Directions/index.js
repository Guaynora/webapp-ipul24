...
function Directions() {
  ...
  useEffect(() => {
    const getCount = async () => {
      const response = await getData(
        `${process.env.REACT_APP_APIURL}/directions/count`,
        token
      );
  ...
  useEffect(() => {
    const getMembers = async () => {
      const response = await getData(
        `${process.env.REACT_APP_APIURL}/directions?_start=${firstPostIndex}&_limit=${postPerPage}`,
        token
      );
  ...
  return (
    <section className="container">
      ...
      <div className="title-button mx-6 mt-4">
        <h3 className="title is-4 m-0">Direcciones</h3>
        <AddButton text="direccion" />
      </div>
      ...
      <div className="table-container p-6">
        ...
        />
        <Table datas={data} type="direction" />
      </div>
    </section>
  );
}
